...
$$
s_c(a) = \underbrace{w_H \sum_{k \in K^H_c} \mathbb{1}\{k \in a\}}_{\text{strong positives},\ w_H=3} + \underbrace{w_M \sum_{k \in K^M_c} \mathbb{1}\{k \in a\}}_{\text{weak positives},\ w_M=1} - \underbrace{\sum_{n \in K^N_c} w_n \mathbb{1}\{n \in a\}}_{\text{vetoes}} + \underbrace{\sum_{P \in K^{CO}_c} w_P \prod_{k \in P} \mathbb{1}\{k \in a\}}_{\text{co-occurrence}}
$$
...
$$
d_H(S_a, S_b) \leq 6 \quad (\text{out of 64 bits})
$$
...
$$
R(a) = \max_{c \in \text{filter}} s_c(a) \cdot e^{-\lambda \Delta t_a}
$$
